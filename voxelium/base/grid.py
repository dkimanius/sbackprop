...
def get_fsc_torch(F1, F2, ang_pix=1, visualize=False):
    ...
    N = F1.shape[-1]
    ind = torch.linspace(-(N - 1) // 2, N // 2 - 1, N)
...
def sample_gaussian_function(coordinates, shape, scale, sample_size=10):
    device = coordinates.device

    shape = shape if isinstance(shape, list) else [shape, shape, shape]
    ls0 = torch.linspace(0, shape[0] - 1, shape[0])
    ls1 = torch.linspace(0, shape[1] - 1, shape[1])
    ls2 = torch.linspace(0, shape[2] - 1, shape[2])
    lattice_meshgrid = torch.stack(torch.meshgrid(ls0, ls1, ls2, indexing='ij'), -1).to(device)

    gaussian = torch.zeros(lattice_meshgrid.shape[:-1], device=device, requires_grad=True)
    input_coordinates = torch.flip(coordinates, dims=(-1,))

    exp_scale = -0.5 / (scale ** 2)
    for idx in range(input_coordinates.shape[0]):  # Loop because of memory constraints
        c = input_coordinates[idx].cpu().numpy()
        o = (np.round(c) - sample_size // 2).astype(int)
        local_lattice = lattice_meshgrid[
            max(0, o[0]):min(shape[0], o[0] + sample_size),
            max(0, o[1]):min(shape[1], o[1] + sample_size),
            max(0, o[2]):min(shape[2], o[2] + sample_size)
        ]
        ...
        gaussian[
            ...
            torch.exp(
                exp_scale *
                (local_lattice - input_coordinates[idx][None, None, None, :]).square().sum(dim=-1)
            )
        )
...
if __name__ == "__main__":
    ...
    while True:
        with torch.no_grad():
            pos = sample_gaussian_function(
                torch.randn(count, 3).to(device) * spread * shape + shape // 2,
                shape,
                size * shape,
                sample_size=int(size * shape * 10)
            )
            neg = sample_gaussian_function(
                torch.randn(count, 3).to(device) * spread * shape + shape // 2,
                shape,
                size * shape,
                sample_size=int(size * shape * 10)
        ...
        plt.imshow(g[shape//2].detach().cpu().numpy())
        plt.show()