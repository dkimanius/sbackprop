...
class RelionDataset:
    def __init__(self, path: str = None, dtype: np.dtype = np.float32, random_subset: str = None):
        self.dtype = dtype
    ...
    def _load_particles(self, particles: dict) -> None:
        ...
        for i in range(nr_particles):
            if self.random_subset is not None:
                if particles["rlnRandomSubset"][i] != self.random_subset:
                    continue
            ...
            self.part_random_subset.append(particles["rlnRandomSubset"][i])
